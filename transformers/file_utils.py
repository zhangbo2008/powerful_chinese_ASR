...
def hf_bucket_url(
    ...
    if revision is None:
        revision = "main"         # 校对号.也加上.
    return HUGGINGFACE_CO_PREFIX.format(model_id=model_id, revision=revision, filename=filename) # 字符串配上路径.
...
def cached_path(
    ...
    if is_remote_url(url_or_filename):
        ...
        output_path = get_from_cache(  #这个函数进行cache中读取或者下载.
            url_or_filename,
            cache_dir=cache_dir,
...
#用sha256来改文件名.
    ...
    if os.path.exists(cache_path) and not force_download:
        return cache_path
#===========下面是文件下载的逻辑, 需要一个文件锁, 这些东西是os底层了.我们跳过.以后有时间细扣.
