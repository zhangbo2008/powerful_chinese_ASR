...
class Wav2Vec2Processor:
    ...
    def as_target_processor(self):
        ...
        Wav2Vec2.  # 这个上下文用于编码标签.
        """ # 首先我们运行with 里面这个函数.创建里层需要的环境context,然后整个函数就是要跑下面这3行.但是因为yield存在只跑到yield,之后就开始运行with 的内部函数了. 运行完.再跑yield下面一行. 你问为什么是这样运行, 没道理. 你点开contextmanager源码.里面告诉你推荐使用方式就是这个.配合yield使用的.
        self.current_processor = self.tokenizer # 设置当前预处理是文本编码
        yield
        self.current_processor = self.feature_extractor # 再设置回音频编码
